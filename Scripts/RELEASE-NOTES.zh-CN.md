...
版本：0.12.0-macos27.2（1331） · Apple Silicon / macOS 27 · 2026-09-21。
...
- **修复 Ice Bar 数值停留在旧截图的问题**：展开时保持原生图标可绘制，每约 3 秒刷新 CPU、温度、网速、风扇和内存等数值；收起后恢复隐藏。
- 关闭或快速切换 Ice Bar 时取消旧刷新任务，丢弃跨开关状态的截图；内部展开操作不会误关 Ice Bar。
...
- `Ice-27-0.12.0-macos27.2-arm64.dmg`：磁盘映像，含应用、许可证和本说明。
- `Ice-27-0.12.0-macos27.2-arm64.zip`：相同应用及说明的 ZIP，二选一即可。
- `Ice-27-0.12.0-macos27.2-source.zip`：本次发行对应源码，含构建脚本和测试。
...
**为了读取实时数值，Ice Bar 展开期间原生菜单栏图标也会展开；关闭 Ice Bar 后恢复隐藏。**

## 验证范围与限制

- 10 组独立测试通过；功能代码延续已验证的 local2 修复。
- 实机验证环境：Apple Silicon、macOS 27.0（26A428）、Xcode 27。
- 验证过图标排列、隐藏/展开、弹窗定位和退出重开恢复；本次连续观察到 CPU、温度、网速、风扇和内存数值变化，重复开关后仍可刷新。
...
发行源码：https://github.com/Lolinux/Ice/tree/v0.12.0-macos27.2
